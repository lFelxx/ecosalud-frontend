 ...
 * Contenido cargado desde GET /api/public/settings → legal.terms
 */
...
export default function TermsPage() {
  ...
  useSeo('Términos y Condiciones | Ecosalud Market', 'Lee los términos y condiciones de uso de la plataforma Ecosalud Market.');

  useEffect(() => {
    axiosClient.get<Record<string, string>>('/public/settings')
      .then(r => setText(r.data['legal.terms'] ?? ''))
      .catch(() => setText(''))
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#F4FAF8', display: 'flex', flexDirection: 'column' }}>
      ...
      <Container maxWidth="md" sx={{ flex: 1, py: { xs: 4, md: 7 } }}>
        <Box sx={{ textAlign: 'center', mb: 5 }}>
          <Box sx={{ width: 52, height: 52, bgcolor: '#FDE8E8', borderRadius: 2.5, display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 2 }}>
            <GavelOutlinedIcon sx={{ color: '#C0392B', fontSize: 26 }} />
          </Box>
          <Chip label="Legal" size="small" sx={{ bgcolor: '#FDE8E8', color: '#C0392B', fontWeight: 700, mb: 1.5 }} />
